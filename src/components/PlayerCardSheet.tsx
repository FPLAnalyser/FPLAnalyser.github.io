    ...
    <div className="fixed inset-0 z-[210] grid place-items-center bg-black/70 p-3 backdrop-blur-sm" onClick={onClose} role="dialog" aria-modal="true">
      <div className="flex max-h-[88dvh] w-full max-w-[600px] flex-col overflow-hidden rounded-2xl border border-line-mid bg-surface-1 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="px-4 pt-3 pb-4">
          {tab === 'rating' ? (
            <Exportable
              title={`${player.web_name} — rating breakdown`}
              toolbar={tabs}
              // The sheet header carries the face, the name and the number, and
              // it sits outside this node — so the export was a set of bars
              // belonging to nobody. Same three facts, drawn into the picture.
              ident={
                <div className="mb-2.5 flex items-center gap-3 rounded-xl border border-line bg-surface-2/50 px-3 py-2.5">
                  <PlayerPhoto
                    code={num(player, 'code')} element={num(player, 'element')}
                    className="w-11 shrink-0 rounded-lg object-cover object-top" style={{ height: 54 }}
                    placeholder={<span className="block w-11 shrink-0 rounded-lg bg-surface-3" style={{ height: 54 }} />}
                  />
                  <div className="min-w-0 flex-1">
                    <div className="truncate text-[17px] font-extrabold tracking-[-0.01em] text-ink">{String(player.web_name)}</div>
                    <div className="text-[12.5px] text-ink-2">{pos} · {teamLabel(String(player.team))} · £{price}m</div>
                  </div>
                  <div className="metallic-num font-display shrink-0 text-[32px] leading-none">{rating ?? '—'}</div>
                </div>
              }
            >
