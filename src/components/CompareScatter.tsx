...
export const COMPARE_LENSES: { id: CompareLens; label: string; tip: string }[] = [
...
function lensPoints(rows: RatingRow[], lens: CompareLens): Pt[] {
  const pts: Pt[] = []
  for (const r of rows) {
...
export function PlayerCompare({ rows, lens, highlightName, onPlayer }: {
  ...
  const pts = useMemo(() => lensPoints(rows, lens), [rows, lens])
  ...
  const axis = lens === 'value' ? ['Rating ↑', 'Price →'] : lens === 'roles' ? ['xA / 90 ↑', 'npxG / 90 →'] : ['Last 4 GW ↑', 'Season →']
  ...
  return (
    <div className="rounded-xl border border-line bg-surface-1 p-4">
      <ScrollTo frac={(X(gold.x) - PAD.l) / (W - PAD.l - PAD.r)}>
        <svg viewBox={`0 0 ${W} ${H}`} className="w-full min-w-[560px]" role="img" aria-label="Player comparison scatter">
          ...
          {pts.filter((d) => d !== gold).map((d) => (
            <circle key={String(d.r.element)} cx={X(d.x)} cy={Y(d.y)} r="4" fill="var(--ink-3)" opacity="0.35" className="cursor-pointer" onClick={() => onPlayer(String(d.r.web_name), num(d.r, 'code'))}>
              <title>{String(d.r.web_name)} ({d.r.team}) — {fmt(d.x)} / {fmt(d.y)} · £{d.r.price}m</title>
            </circle>
          ))}
          {labelled.map((d) => (
          ...
          <circle cx={X(gold.x)} cy={Y(gold.y)} r="6" fill="var(--accent)" stroke="var(--surface-1)" strokeWidth="1.5" className="cursor-pointer" onClick={() => onPlayer(String(gold.r.web_name), num(gold.r, 'code'))}>
            <title>{String(gold.r.web_name)} ({gold.r.team}) — {fmt(gold.x)} / {fmt(gold.y)} · £{gold.r.price}m</title>
          </circle>
      ...
      <div className="mt-2 text-xs text-ink-3">
        ...
        {lens === 'roles' && 'Scorers sit right, creators sit high; top-right is the rare both-axis elite. '}
        Gold follows your search. Tap any dot to open the player.
      </div>
    </div>
  )
}
